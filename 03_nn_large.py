...
# LeNet-like model (large)
model_large = Sequential()
model_large.add(Conv2D(32, (12, 5), activation="relu", input_shape=(12, 30, 1)))
model_large.add(MaxPool2D((1, 2)))
model_large.add(Conv2D(64, (1, 5), activation="relu"))
model_large.add(MaxPool2D((1, 2)))
model_large.add(Flatten())
model_large.add(Dense(256, activation="relu"))
model_large.add(Dense(256, activation="relu"))
model_large.add(Dense(1, activation="sigmoid"))
model_large.compile(loss="binary_crossentropy", optimizer="Adam")
...
acc_df, logloss_df, auc_df = fit_climate(model_large, epochs)
acc_df.to_csv("./output/nn_large_acc.csv", index=False)
logloss_df.to_csv("./output/nn_large_logloss.csv", index=False)
auc_df.to_csv("./output/nn_large_auc.csv", index=False)
